...
```{r}
score_tree <- rpart(data = dt_students,
                     ...
                     extracurricular_participation, method = 'anova')

prp(score_tree, digits = -3)
```
...
```{r}
score_tree_min_cp <- score_tree$cptable[which.min(score_tree$cptable[, "xerror"]), "CP"]
model_tree <- prune(score_tree, cp = score_tree_min_cp )
prp(score_tree,digits = -3)
```
...
```{r}
scores_tree_pred <- predict(score_tree, 
                            dt_students[, c("age",
                                            "gender", 
                                            "study_hours_per_day", 
                                            "social_media_hours", 
...
scores_tree_mse <- mean((abs(dt_students$exam_score - scores_tree_pred)^2))
